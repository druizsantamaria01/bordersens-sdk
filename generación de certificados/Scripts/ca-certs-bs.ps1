...
$_rootCertCommonName      = "Azure IoT CA BorderSens Root CA"
$_rootCertSubject         = "CN=$_rootCertCommonName"
$_intermediateCertCommonName = "Azure IoT CA BorderSens Intermediate {0} CA"
$_intermediateCertSubject    = "CN=$_intermediateCertCommonName"

$rootCACerFileName          = "./RootCA.cer"
$rootCAPemFileName          = "./RootCA.pem"
$intermediate1CAPemFileName = "./Intermediate1-{0}.pem"
$intermediate2CAPemFileName = "./Intermediate2-{0}.pem"
$intermediate3CAPemFileName = "./Intermediate3-{0}.pem"
...
$edgeIotHubOwnerCA          = Join-Path $edgePublicCertDir "azure-iot-bordersens.root.ca.cert.pem"
...
{
    Write-Host ("Testing if any BorderSens certificates have already been installed...")
    $certInstalled = $null
    try
    {
    ...
    if ($NULL -ne $certInstalled)
    {
        ...
        $cleanup_msg += " - Navigate to Certificates -> Trusted Root Certification Authority -> Certificates. Remove certificates issued by 'Azure IoT CA Bodersens*'.$nl"
        $cleanup_msg += " - Navigate to Certificates -> Intermediate Certificate Authorities -> Certificates. Remove certificates issued by 'Azure IoT CA Bodersens*'.$nl"
        $cleanup_msg += " - Navigate to Certificates -> Local Computer -> Personal. Remove certificates issued by 'Azure IoT CA Bodersens*'.$nl"
        $cleanup_msg += "$nl$nl"
...
function New-CACertsSelfsignedCertificate([string]$commonName, [object]$signingCert, [bool]$isASigner=$true)
{
    ...
    Write-Warning ("Generating certificate CN={0} which is for prototyping." -f $commonName)
...
# Create Olnly Root Certificate
function New-CACertsCertChainOnlyRoot([Parameter(Mandatory=$TRUE)][ValidateSet("rsa","ecc")][string]$algorithm)
{
    ...
    Write-Output 'XXXXXXXXXXXXXXXXXXXXXXX'
    Write-Output $rootCACert
    Write-Output 'XXXXXXXXXXXXXXXXXXXXXXX'
...
# Create Olnly Root Certificate
function New-CACertsCertChainOnlyInermediate([Parameter(Mandatory=$TRUE)][ValidateSet("rsa","ecc")][string]$algorithm)
{
    # Store the algorithm we're using in a file so later stages always use the same one (without forcing user to keep passing it around)
    Set-Content $algorithmUsedFile $algorithm
...
function New-CACertsCertChainOnlyRoot([Parameter(Mandatory=$TRUE)][ValidateSet("rsa","ecc")][string]$algorithm)
{
    ...
    Write-Output 'RootCertSubject:'("RootCertSubject: {0}" -f $_rootCertSubject )
...
# Create Olnly Root Certificate
function New-CACertsCertChainOnlyInermediate([Parameter(Mandatory=$TRUE)][ValidateSet("rsa","ecc")][string]$algorithm,
                                             [Parameter(Mandatory=$TRUE)][string]$intermediateName)
{
    # Store the algorithm we're using in a file so later stages always use the same one (without forcing user to keep passing it around)
    $certIntermediateCommunName =  ($_intermediateCertCommonName -f $intermediateName)
    Set-Content $algorithmUsedFile $algorithm
    $rootCACert = Get-CACertsCertBySubjectName $_rootCertSubject
    $intermediateCert1 = New-CACertsIntermediateCert ($certIntermediateCommunName -f $intermediateName) $rootCACert ($intermediate1CAPemFileName -f $intermediateName)
    Write-Output 'IntermediateCertSubject:'("IntermediateCertSubject: {0}" -f $certIntermediateCommunName )
    Write-Output $intermediateCert1
    Write-Host "Success"
}

function New-CACertsDeviceFromParent()
{
    param([Parameter(Mandatory=$true)][string]$deviceName, 
          [Parameter(Mandatory=$true)][string]$signingCertSubject,
          [Parameter(Mandatory=$true)][System.Security.SecureString]$certPassword,
          [bool]$isEdgeDevice=$false
    )
